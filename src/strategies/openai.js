...
async function humanizeWithOpenAI(data, options = {}) {
  ...
  // FIX: OpenAI SDK v4 in CJS exports the class directly; in some bundlers it's under .default
  ...
  // FIX: guard against empty choices (content filter, quota exceeded, etc.)
  if (!response.choices || response.choices.length === 0) {
    throw new Error("OpenAI returned no choices. The request may have been filtered or the model is unavailable.");
  }
